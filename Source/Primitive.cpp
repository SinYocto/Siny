...
Cube::Cube()
{
	...
	positionData = pos;
	uvData = uv;
	normalData = new Vector3[24];
	tangentData = new Vector3[24];
	bitangentData = new Vector3[24];
	indexData = indices;
...
Cylinder::Cylinder(float topR, float bottomR, float cylinderHeight, int segmentsWidth, int segmentsHeight, D3DXCOLOR cylinderColor)
{
	...
	segmentsW = segmentsWidth;
	segmentsH = segmentsHeight;
	color = cylinderColor;
...
Sphere::Sphere(float r, int sw, int sh)
{
	radius = r;
	segmentsW = sw;
	segmentsH = sh;
